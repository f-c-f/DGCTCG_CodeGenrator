...
int ILTransStatementForCBMC::translateStmtIf(const Statement* stmt)
{
	const StmtIf* stmtTemp = static_cast<const StmtIf*>(stmt);
    
    string conditionExpression = stmtTemp->condition ? stmtTemp->condition->expressionStr : "";
    if(ILTranslatorForCBMC::configTautology)
	{
        statementStr += "char __tautologyTempVariable__v" + to_string(ILTranslatorForCBMC::tautologyTempVariableCount) + " = (" + conditionExpression + ") != 0;\n";
        conditionExpression = "__tautologyTempVariable__v" +
            to_string(ILTranslatorForCBMC::tautologyTempVariableCount);
		statementStr += "assert(" + conditionExpression + ");//For Tautology Branch Detection\n";
        ILTranslatorForCBMC::tautologyTempVariableCount++;

	}

	statementStr += "if (";
	statementStr += conditionExpression;
	statementStr += ") {\n";
	...
	{
	    statementStr += "assert(0);//For Unreachable Branch Detection\n";
        statementStr += "assert(1);//For Unreachable Branch Detection\n";
	}
...
int ILTransStatementForCBMC::translateStmtElse(const Statement* stmt)
{
...
int ILTransStatementForCBMC::translateStmtElseIf(const Statement* stmt)
{
...
int ILTransStatementForCBMC::translateStmtWhile(const Statement* stmt)
{
...
int ILTransStatementForCBMC::translateStmtDoWhile(const Statement* stmt)
{
...
int ILTransStatementForCBMC::translateStmtFor(const Statement* stmt)
{
...
int ILTransStatementForCBMC::translateStmtContinue(const Statement* stmt)
{
	statementStr += "continue;\n";
	return 1;
}

int ILTransStatementForCBMC::translateStmtSwitch(const Statement* stmt)
{
	...
	if (!lastIsDefault) {
        ...
        if(ILTranslatorForCBMC::configBranchCover)
        {
	        Instrument_BC::InsertStatementInBlockBranchCoverageCollection(stmt, "switch_default_" + stmtTemp->condition->expressionStr, statementStr);
        }
	}
...
int ILTransStatementForCBMC::translateStmtCase(const Statement* stmt)
{
...
int ILTransStatementForCBMC::translateStmtDefault(const Statement* stmt)
{
