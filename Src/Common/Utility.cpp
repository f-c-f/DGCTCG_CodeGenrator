...
std::vector<std::string> stringSplit(const std::string& s, const std::vector<std::string>& seperator)
{
    ...
    {
        end = s.find(seperator[i], start);
        if(end == string::npos)
        {
            break;
        }
        ret.push_back(s.substr(start, end - start));
        start = end + seperator[i].length();
    }
...
#endif
	for(int i=0; dir[i] != 0 && i< FILE_PATH_MAX; i++){
        if(dir[i] == '\\'){            
            dir[i]='/';
        }
	}
...
string getVariableExpRootVarName(string exp)
{
    string ret;
    ...
    // Check if the first character is a valid start for a C variable name
    if (isalpha(exp[i]) || exp[i] == '_') {
        ret += exp[i++];
        j++;
        // Continue capturing valid characters for a variable name
        while (exp[i] && (isalnum(exp[i]) || exp[i] == '_')) {
            ret += exp[i++];
            j++;
        }
    }
