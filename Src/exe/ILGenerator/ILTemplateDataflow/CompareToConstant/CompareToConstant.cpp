...
{
    if(actorInfo->outports.empty() || actorInfo->inports.size() != 1)
        return "";

    string op = actorInfo->getParameterValueByName("Relop");
    if(op == "~=")
        op = "!=";
    string value = actorInfo->getParameterValueByName("Const");
...
vector<StmtBatchCalculation> transExecBatchCalculation()
{
    ...
    string para = actorInfo->getParameterValueByName("Relop");
    ...
    stmtInput2.defaultValue = expParser.parseExpression(actorInfo->getParameterValueByName("Const"));
