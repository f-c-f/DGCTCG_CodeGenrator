...
bool TCGHybrid::hybridSliceStallTrackingReady = false;
TCGHybrid::CoverageInfo TCGHybrid::hybridSliceLastCovSnapshot = {};
long long TCGHybrid::hybridSliceLastProgressClockTicks = 0;

namespace {
constexpr long long kHybridCoverageStallSliceClockTicks =
	static_cast<long long>(10) * static_cast<long long>(CLOCKS_PER_SEC);
}
...
void TCGHybrid::initTCGHybrid2()
{
    ...
    hybridSliceLastCovSnapshot.munitCov = BitmapCov::munitCov;
    hybridSliceLastCovSnapshot.branchCov = BitmapCov::branchCov;
    hybridSliceLastCovSnapshot.condCov = BitmapCov::condCov;
    hybridSliceLastCovSnapshot.decCov = BitmapCov::decCov;
    hybridSliceLastCovSnapshot.mcdcCov = BitmapCov::mcdcCov;
    hybridSliceLastProgressClockTicks = static_cast<long long>(clock());
    hybridSliceStallTrackingReady = true;
}

void TCGHybrid::hybridEmitSliceRequestJsonForStall()
{
    const string outDir = exePath + "/SliceOut";
    createFolder(outDir);
    string idxPart;
    ...
    string js = string("{\"trigger\":\"hybrid_coverage_stall_10s\",")
        + "\"clock_ticks_at_emit\":" + to_string(static_cast<long long>(clock())) + ","
        + "\"aggregate_coverage\":{"
        + "\"munitCov\":" + to_string(BitmapCov::munitCov)
        + ",\"branchCov\":" + to_string(BitmapCov::branchCov)
        + ",\"condCov\":" + to_string(BitmapCov::condCov)
        + ",\"decCov\":" + to_string(BitmapCov::decCov)
        + ",\"mcdcCov\":" + to_string(BitmapCov::mcdcCov)
        + "},\"uncovered_branch_bitmap_indices\":["
        + idxPart
        + "],\"note\":\"Pair with obligation_map.json (bitmap_index). IL slicer runs outside this binary.\"}";
    const string path = outDir + "/hybrid_coverage_stall_slice_request.json";
    writeFile(path, js);
    appendFile(exePath + "/log.txt",
        "[HybridSlice] emitted " + path + " (aggregate munit/branch/cond/dec/mcdc counts unchanged for >=10s)\n");
    cout << "[HybridSlice] wrote " << path << endl;
}

void TCGHybrid::hybridCoverageStallSliceTickAfterStep()
{
    if (!hybridSliceStallTrackingReady)
        return;
    CoverageInfo cur;
    cur.munitCov = BitmapCov::munitCov;
    cur.branchCov = BitmapCov::branchCov;
    cur.condCov = BitmapCov::condCov;
    cur.decCov = BitmapCov::decCov;
    cur.mcdcCov = BitmapCov::mcdcCov;
    const bool progressed =
        cur.munitCov > hybridSliceLastCovSnapshot.munitCov
        || cur.branchCov > hybridSliceLastCovSnapshot.branchCov
        || cur.condCov > hybridSliceLastCovSnapshot.condCov
        || cur.decCov > hybridSliceLastCovSnapshot.decCov
        || cur.mcdcCov > hybridSliceLastCovSnapshot.mcdcCov;
    if (progressed) {
        hybridSliceLastCovSnapshot = cur;
        hybridSliceLastProgressClockTicks = static_cast<long long>(clock());
        return;
    }
    const long long now = static_cast<long long>(clock());
    if (now - hybridSliceLastProgressClockTicks >= kHybridCoverageStallSliceClockTicks) {
        hybridEmitSliceRequestJsonForStall();
        hybridSliceLastProgressClockTicks = now;
    }
}
 ...
 * 6. 本步末尾：若聚合覆盖计数连续约 10s 无增长，写出 SliceOut/hybrid_coverage_stall_slice_request.json  
 *
...
bool TCGHybrid::stepTCGHybrid()
{
    ...
    /* 覆盖约 10s 无进展则写出切片请求（与 minimize/randomRun/STCG 等本步工作全部完成后再判定） */
    TCGHybrid::hybridCoverageStallSliceTickAfterStep();
