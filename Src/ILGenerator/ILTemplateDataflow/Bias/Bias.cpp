...
string transExecExpression()
{
    ...
    string biasValue = actorInfo->getParameterValueByName("Bias");

    if(!biasValue.empty() && !actorInfo->inports.empty())
    {
        expressionStr += getCGTActorExeTransSourceOutportVariableNameByName(actorInfo->inports[0]->sourceOutport) + " + " + biasValue;
    }
    expressionStr += ";";
...
vector<StmtBatchCalculation> transExecBatchCalculation()
{
    ...
    stmtInput2.defaultValue = expParser.parseExpression(actorInfo->getParameterValueByName("Bias"));
